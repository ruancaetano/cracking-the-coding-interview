...
import "github.com/ruancaetano/cracking-the-coding-interview/golang/shared/graph"
...
func generateNodes(n int) []*graph.Node[int] {
	result := make([]*graph.Node[int], n)
	for i := range n {
		result[i] = graph.NewNode(i)
	}

	return result
}

func buildGraphs(nodes []*graph.Node[int]) *graph.Graph[int] {
